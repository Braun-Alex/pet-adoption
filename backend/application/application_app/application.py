...
@application_router.post(CREATE_APPLICATION_URL, response_model=int)
...
def get_application(shelter_id: int = None, user_id: int = None, application_id: int = None):
    ...
    if shelter_id is not None: 
        ...
        applications = application_service.get_application_by_id(application_id=application_id)
    logger.info(f"Get applications from DB: {applications}, {isinstance(applications, ApplicationOut)}")

    return applications if applications else [ApplicationOut]

@application_router.post(UPDATE_STATUS_URI, response_model=Optional[ApplicationOut])

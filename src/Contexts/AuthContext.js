...
export const AuthProvider = ({ children }) => {
    ...
    const tryRefreshAccessToken = async () => {
        ...
        try {
            ...
            const response = await axios.get(`${process.env.BACKEND_HOSTNAME}:${process.env.BACKEND_PORT}/api/v1/token/refresh`);
    ...
    const getUserData = async () => {
        const user = await getData(`${process.env.BACKEND_HOSTNAME}:${process.env.BACKEND_PORT}/api/v1/users/profile`);
        if (user) {
            return {
                userID: user.id,
    ...
    const getShelterData = async () => {
        const shelter = await getData(`${process.env.BACKEND_HOSTNAME}:${process.env.BACKEND_PORT}/api/v1/shelter/profile`);
        if (shelter) {
            return {
    ...
    const getUserApplications = async (user) => {
        try {
            const applicationsResponse = await axios.get(`${process.env.BACKEND_HOSTNAME}:${process.env.BACKEND_PORT}/api/v1/applications/get/?user_id=${user.userID}`);
            const applications = applicationsResponse.data;
            for (const application of applications) {
                const animalResponse = await axios.get(`${process.env.BACKEND_HOSTNAME}:${process.env.BACKEND_PORT}/api/v1/animals/animal/${application.animal_id}`);
